...
- 구현 메모 (CPC 확장 검색 로직)
    - **1차:** SerpAPI로 인용 특허의 **CPC 코드 추출**
    - **2차:** 각 CPC를 **LLM(GPT-4o-mini)** 로 기술명 키워드로 변환
    - **3차:** 변환된 키워드로 **SerpAPI 재검색**, 추가 특허의 CPC 수집
    - **4차:** 전체 CPC 분포 기반으로
        
        → (Originality = 1 - Σ(pₖ²)) 계산
...
- **하이브리드 검색 로직 (RAG + Web Search)**
    - RAG 검색
        ...
        > 현재는 이 PDF를 도메인 핵심 개념을 정착시키는 시드 지식으로 활용하지만, 향후에는 최신 특허·데이터시트·시장 리포트처럼 **정량적이고 최신성이 높은 1차 자료 중심으로 확장할 계획입니다.**
        > 
    - Tavily Web Search
        - **쿼리 3가지**:
            1. **시장 규모**: `"{tech} {keyterm} market size 2024 2025 billion USD"`
            2. **성장률**: `"{tech} {keyterm} CAGR growth rate forecast 2024-2028"`
            3. **응용 분야**: `"{tech} application market forecast"`
    - 점수 산정 방식 (LLM 종합)
        - **Abstract**: 특허 기술 내용 (핵심)
        - **RAG 문서**: 관련 시장 보고서 참고
        - **Tavily 결과**: 구체적 수치 우선 활용 (예: "602억 달러", "CAGR 11%")

---

### 3. 적합성 **지표 (MarketSizeGrowthAgent)**

- 독창성 점수(Originality)와 시장성 점수(Market)를 종합하여 **특허 기술의 최종 지속가능성 점수**를 산정하고, **LLM-as-a-Judge**를 통해 투자 의사결정을 지원합니다.
...
- **(1차 평가)**
    ...
    - **가중치 설계 근거:**
        - **독창성 55%**: 기술적 독창성이 높은 편향을 보임 (→ 독창성 점수 선형 정규화)
...
- **(2차 평가)**
    - GPT-4 Judge의 역할
        ...
        2. **맥락 이해**: 점수 조합이 의미하는 바 파악
        3. **전략 제시**: 구체적인 액션 아이템 제공
        4. **등급 재평가**: 필요 시 계산된 등급 조정
